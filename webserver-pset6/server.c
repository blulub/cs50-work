...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // get pointer to the start of request-target
            char* start_of_requestTarget = strchr(line, '/');
            if (start_of_requestTarget == NULL)
            {
                error(400);
                continue;
            }
            // get pointer to the start of http version
            char* start_of_HTTP = strstr(line, "HTTP");
            // index of start of request-target is the pointer to the start minus the pointer to the start of line
            int index_start_requestTarget = (start_of_requestTarget - line);
            if (index_start_requestTarget != 4)
            {
                error(400);
                continue;
            }
            // length of request-target including \0 is http poitner address minus start of request target address
            int length_requestTarget = (start_of_HTTP - start_of_requestTarget);
            // index of the end of request-target is index of the start + length of request target minus one due to space
            int index_end_requestTarget = (index_start_requestTarget + length_requestTarget - 1);
            
            // set an array of chars of length request-target
            char requestTarget[length_requestTarget];
            // make the last character null
            requestTarget[length_requestTarget - 1] = '\0'; 
            // start for loop to copy the substring requestTarget from line into requestTarget
            for (int i = index_start_requestTarget, j = 0 ; i < index_end_requestTarget ; i++, j++)
            {
                requestTarget[j] = line[i];
            }
            
            // find the end indexof the string
            
            int index_end_line = strlen(line);
            // find start index of HTTPversion
            int index_start_HTTP = (start_of_HTTP - line);
            int length_HTTPversion = (index_end_line - index_start_HTTP) + 1;
            // initiate char array to contain HTTPversion
            char HTTPversion[length_HTTPversion];
            HTTPversion[length_HTTPversion - 1] = '\0';
            // get the HTTP-version by running loop
            for (int i = index_start_HTTP, j = 0 ; i < index_end_line ; i++, j++)
            {
            ...
            if ((strchr(requestTarget, '.') == NULL) || (requestTarget[0] != '/'))
            {
                error(501);
                continue;
            }
            ...
            // declare the char arrays outside of the loop to use later
            char query[20];
            char absolute_target[50];
            char path[200];

            // see if there is a query in request
            char* start_query = strchr(requestTarget, '?') ;
            if (start_query == NULL)
            {
                // if no query, set query to ""
                query[0] = '\0';
                // set absolute_target to be same as requestTarget for ease of use later
                strcpy(absolute_target, requestTarget);

                // find the length path using requestTarget
                int length_path = (strlen(root) + strlen(requestTarget)) + 1;
            ...
            {
                // get out the query substring
                char* start_requestTarget = requestTarget;
                int index_start_query = (start_query - start_requestTarget) + 1;
                int length_query = (strlen(requestTarget) - index_start_query) + 1;

                
                query[length_query - 1] = '\0';
                for (int i = index_start_query, j = 0; i < strlen(requestTarget); i++, j++)
                {
                    query[j] = requestTarget[i];
                }



                // make the absolute target not inclue the query, so we can use in filepath
                char* start_qmark = strchr(requestTarget, '?');
                if (start_qmark != NULL)
                {
                    int index_qmark = start_qmark - start_requestTarget;
                    absolute_target[index_qmark] = '\0';
                    strncpy(absolute_target, requestTarget,index_qmark);
                }

                // use absolute target in path instead of request target that contains query
                int length_path = (strlen(root) + strlen(absolute_target)) + 1;
                path[length_path -1] = '\0';
                path[0] = '\0';
                strcat(path, root);
                strcat(path, absolute_target);
            }
            ...
            char* start_ext = strchr(absolute_target, '.');
            char* absolute_target_start = absolute_target;
            int index_start_ext = start_ext - absolute_target_start + 1;
            int lengthExt = strlen(absolute_target) - index_start_ext + 1;
            char extension[lengthExt];
            extension[lengthExt - 1] = '\0';
            for (int i = index_start_ext, j = 0; i < strlen(absolute_target); i++, j++)
            {
                extension[j] = absolute_target[i];
            }
...
bool error(unsigned short code)
{
    ...
    int length = sprintf(content, template, code, phrase, code, phrase); // puts template into content to be filled with code phrase code phrase
    ...
    if (dprintf(cfd, "Connection: close\r\n") < 0) // dprintf returns the number of chars written
    {
        return false;
    }
...
ssize_t load(void)
{
    ...
    while (true)
    {
        ...
        ssize_t octets = fread(buffer, sizeof(octet), OCTETS, file); // fread into buffer, 512 times 8 bits from file
        ...
        if (octets > 0)
        {
            ...
            memcpy(body + size, buffer, octets); // memcpy (void* destination, const void* source, size_t num
            size += octets;
        }
...
const char* lookup(const char* extension)
{
    char* copy = malloc(sizeof(char) * strlen(extension));
    strcpy(copy, extension);
    for (int i = 0; i < strlen(extension); i++)
    {
        if (!islower(copy[i]))
        {
            // make a lowercase copy of extension to compare
            copy[i] = tolower(copy[i]);
        }
    }
    
    if (strcmp(copy, "css") == 0)
    {
        free(copy);
        return "text/css";
    }
    else if (strcmp(copy,"html") == 0)
    {
        free(copy);
        return "text/html";
    }
    else if (strcmp(copy,"gif") == 0)
    {
        free(copy);
        return "image/gif";
    }
    else if (strcmp(copy,"ico") == 0)
    {
        free(copy);
        return "image/x-icon";
    }
    else if (strcmp(copy,"jpg") == 0)
    {
        free(copy);
        return "image/jpeg";
    }
    else if (strcmp(copy,"js") == 0)
    {
        free(copy);
        return "text/javascript";
    }
    else if (strcmp(copy,"png") == 0)
    {
        free(copy);
        return "image/png";
    }
